...
DatabaseTools.prototype.select = function(table, id, callback) {
...
// Special function for false_directories
// Returns the id given the fake path
DatabaseTools.prototype.altselect = function(table, value, callback) {
  var credentialInstance = this._credentialInstance; // Assign to local variable
  var con = mysql.createConnection({
    host: credentialInstance.getServer(),
    user: credentialInstance.getUser(),
  ...
  // Conn Queries Go Here
  con.query(`SELECT id FROM ${table} WHERE fake_path = '${value}'`, function(err,rows){
    if(err){
      throw err;
    }
    callback(rows);
